...
function Request-TuiCredential {
    ...
    if ($Targets.Count -eq 1) {
        ...
        if ([string]::IsNullOrWhiteSpace($computerName) -or
            ...
            if ($computerName -ne [string]$Targets[0].Address) { $Targets[0].ComputerName = $computerName }
            if ($resolved.ConnectionHost -ne [string]$Targets[0].Address) { $Targets[0].VerifiedConnectionHost = $resolved.ConnectionHost }
        }
    }
    $computerQualifier = if ([string]::IsNullOrWhiteSpace($computerName)) { $null } else { $computerName.Split('.')[0] }
    ...
    $null = Write-WinRMAuditEvent -StorePath $script:StorePath -Event 'CredentialEntered' -Data @{ User = $script:credential.UserName; Targets = @($Targets.Address) }
...
function Enter-TuiRemoteShell {
    ...
    try {
        ...
        $null = Invoke-Command -Session $session -ErrorAction SilentlyContinue -ScriptBlock {
            try {
...
try {
    ...
    while ($running) {
        ...
        Start-ConsoleTuiFrame
        Write-ConsoleTuiLine (Format-ConsoleTuiText ' WinRM Fleet Console ' $width) Black Cyan
        ...
        Write-ConsoleTuiLine (Format-ConsoleTuiText ' D scan  F filter  Space select  I inventory  X script  Enter shell  ? help  Q quit ' $width) Black DarkGray
        Complete-ConsoleTuiFrame
        ...
        switch ($key.Key) {
            ...
            'S' { Edit-TuiSettings }
            'R' { $config = Get-WinRMManagerConfig -StorePath $StorePath; $inventory = @(Get-WinRMInventory -StorePath $StorePath); $lastMessage = 'State reloaded.' }
            'Oem2' { Show-TuiHelp }
            'Q' { $running = $false }
        }
    }
}
finally { Restore-ConsoleTui }
