...
### Added
- `Block-DomainFirewall.ps1`: resolves a DNS name and creates or refreshes an
  outbound Windows Defender Firewall rule for all returned IPv4/IPv6 addresses.
- `Block-DomainHosts.ps1`: blocks an exact DNS name in the Windows hosts file,
  preserves unrelated aliases, creates a timestamped backup, and clears the DNS
  client cache.
